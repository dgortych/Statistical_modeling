...
#zad1
model31 <- lm(MEAN.ANNUAL.RAINFALL ~RECORD.LENGTH + log(RECORD.LENGTH) + RECORD.LENGTH^2 ,data = train)  
RMSE31 <- rmse(train$MEAN.ANNUAL.RAINFALL,model31$fitted.values) #224.5645

#zad2

RMSE31_test <- rmse(test$MEAN.ANNUAL.RAINFALL,predict(model31,test)) #258.2122

#zad3 

model32 <- lm(MEAN.ANNUAL.RAINFALL ~MAX.RAINFALL + log(MAX.RAINFALL) + MAX.RAINFALL^2 ,data = train) 
RMSE32 <- rmse(train$MEAN.ANNUAL.RAINFALL,model32$fitted.values) #116.2342

#zad4

RMSE32_test <- rmse(test$MEAN.ANNUAL.RAINFALL,predict(model32,test)) #111.0829

#zad5

model33 <- lm(MEAN.ANNUAL.RAINFALL ~MAX.RAINFALL + log(MAX.RAINFALL) + MAX.RAINFALL^2 + RECORD.LENGTH + log(RECORD.LENGTH) + RECORD.LENGTH^2 ,data = train)  
RMSE33 <- rmse(train$MEAN.ANNUAL.RAINFALL,model33$fitted.values) #114.8158

#zad6

RMSE33_test <- rmse(test$MEAN.ANNUAL.RAINFALL,predict(model33,test)) #109.8948
...
base_model <- lm(MEAN.ANNUAL.RAINFALL ~ NULL ,data = train) 
expl <- train[c(3,5,9,15)]


step <- step(base_model,as.formula(expl),direction = "forward")
step

#zad8

model34 <- lm(formula = MEAN.ANNUAL.RAINFALL ~ MAX.RAINFALL + MEAN.ANNUAL.AIR.TEMP, 
    data = train)

RMSE34 <- rmse(train$MEAN.ANNUAL.RAINFALL,predict(model34)) #117.1096
RMSE34

RMSE34_test <- rmse(test$MEAN.ANNUAL.RAINFALL,predict(model34,test)) #104.5527
RMSE34_test
